...
Both figures embed **real frames** from the 053243 series (frame 500) at the
corresponding pipeline stage, extracted by `figures/make_thumbnails.py` and
regenerated by `figures/make_figures.py`. The SVGs are self-contained (thumbnails
are inlined as base64) and editable in Inkscape or Illustrator.
