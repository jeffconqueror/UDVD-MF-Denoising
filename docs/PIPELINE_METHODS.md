...
**Figures**
- Figure 1 — [pipeline network graph](figures/fig1_pipeline.svg) ([PNG](figures/fig1_pipeline.png)) — data tensors, modules and the expanded sub-networks
- Figure 2 — [classifier network graph](figures/fig2_classifier.svg) ([PNG](figures/fig2_classifier.png)) — Swin-Tiny feature volumes, dihedral TTA, block internals
- Figure 3 — [compact classifier architecture](figures/fig3_classifier_arch.svg) ([PNG](figures/fig3_classifier_arch.png)) — the same model in the conventional layer-slab style, single panel; use in place of Figure 2 where column width is tight

All three figures embed **real frames** from the 053243 series (frame 500) at the
corresponding pipeline stage, extracted by `figures/make_thumbnails.py` and
regenerated by `figures/make_figures.py` and `figures/make_fig3.py`. The SVGs are
self-contained (thumbnails are inlined as base64) and editable in Inkscape or
Illustrator.
...
> **The UMVD > UDVD-MF preference is currently qualitative, not measured.** What exists
> for the `053243` video is:
>
> | Model | PSNR vs. noisy input | SSIM vs. noisy input | uPSNR | val loss |
> |---|---:|---:|---:|---:|
> | UDVD-MF, pass 1 | 18.98 | 0.106 | **23.86** | — |
> | UDVD-MF, pass 2 | 40.89 | 0.940 | **28.64** | — |
> | UMVD (best, epoch 13) | — | — | **not computed** | 0.014519 |
>
> These columns are **not comparable across models.** UMVD's `val_loss` is the MSE
> against the *noisy* centre frame — the self-supervised training objective — and is not
> a quality metric at all. Nor are the PSNR/SSIM columns: with no clean reference they
> are computed against the noisy input, so a *higher* value means the denoiser changed
> the image *less*. UDVD-MF pass-2's PSNR of 40.89 says it barely denoised, not that it
> denoised well. **Do not quote PSNR or SSIM in the paper.**
>
> The only defensible metric here is **uPSNR** (Marcos Morales et al., ICML 2023), the
> unsupervised estimator implemented at `utils/metrics.py:uMSE_uPSNR`. It is recorded for
> both UDVD-MF passes but was never run on UMVD, because `uMSE_uPSNR(ds, net)` expects
> the UDVD multi-frame `net` interface and UMVD's `Denoiser` takes
> `(n_frames, 1, H, W)` — an adapter is needed, not a new experiment.
>
> **To make this claim publishable:** wrap the UMVD `Denoiser` to match that interface
> and report uPSNR for UMVD and UDVD-MF on the same video, same frames, same seed. Until
> then the honest phrasing in the paper is that UMVD was *selected by visual inspection
> of lattice-fringe retention*, with no quantitative comparison — which is defensible if
> stated, and indefensible if implied otherwise.
